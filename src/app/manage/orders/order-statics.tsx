...
export default function OrderStatics({
  ...
  return (
    <Fragment>
      ...
      >
        <DialogContent className='max-h-full overflow-auto max-w-xl!' aria-describedby={undefined}>
          ...
          return (
            ...
            >
              ...
              <Separator
                ...
                className={cn('shrink-0 grow h-auto', {
                ...
                <div className='flex flex-col gap-2'>
                  <TooltipProvider>
                    <Tooltip>
                      ...
                      <TooltipContent>
                        {getVietnameseOrderStatus(OrderStatus.Pending).message}: {countObject[OrderStatus.Pending] ?? 0} đơn
                      </TooltipContent>
                    </Tooltip>
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent>
                        {getVietnameseOrderStatus(OrderStatus.Processing).message}: {countObject[OrderStatus.Processing] ?? 0}{' '}
                        đơn
                      </TooltipContent>
                    </Tooltip>
                    <Tooltip>
                      ...
                      <TooltipContent>
                        {getVietnameseOrderStatus(OrderStatus.Delivered).message}: {countObject[OrderStatus.Delivered] ?? 0} đơn
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                </div>
      ...
      <div className='flex justify-start items-end gap-4 flex-wrap py-4'>
        {OrderStatusValues.map((status) => (
          <Badge variant='secondary' key={status}>
            {getVietnameseOrderStatus(status).message}: {statics.status[status] ?? 0}
          </Badge>
        ))}
      </div>
    </Fragment>
