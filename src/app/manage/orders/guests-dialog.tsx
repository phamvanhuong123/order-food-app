...
import { useListGuest } from '@/queries/useAccount'
...
export default function GuestsDialog({ onChoose }: { onChoose: (guest: GuestItem) => void }) {
  ...
  const listGuestQuery = useListGuest({fromDate,toDate})
  const data: GetListGuestsResType['data'] = listGuestQuery.data?.payload.data ||  []
            ...
            <div className='flex items-center justify-end space-x-2 py-4'>
              ...
              <div>
                <AutoPagination
                  ...
                  isLink={false}
                  onChangePage={(number : number) => setPagination({pageIndex : number -1,pageSize :PAGE_SIZE })}
                />
              </div>
            </div>
          </div>
