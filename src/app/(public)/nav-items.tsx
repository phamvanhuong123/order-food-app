...
export default function NavItems({ className }: { className?: string }) {
  ...
  return <>
    {menuItems.map(menu => {
      ...
      const canShow = menu.roles === undefined && !menu.hideWhenLogin || (!role && menu.hideWhenLogin)
      if(isAuth || canShow) return <Link href={menu.href} key={menu.href} className={className}>
        {menu.title}
      </Link>
    })}
   {role &&<div onClick={handleLogout} className={cn(className,'cursor-pointer')}>Đăng xuất</div>} 
