...
export async function POST(request: NextRequest) {
  ...
  try {
    const { payload } = await guestApiRequest.refreshToken({
      refreshToken: refreshToken,
    });

    const decodeAccessToken = decode(payload.data.accessToken) as TokenPayload;
    const decodeRefreshToken = decode(
